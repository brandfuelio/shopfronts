...
async function runTests() {
  ...
  try {
    const response = await axios.get(`${API_URL}/performance/report`, {
      headers: { Authorization: `Bearer ${ADMIN_TOKEN}` },
    });
    results.push({
  ...
  try {
    ...
    const response1 = await axios.get(`${API_URL}/products`);
    const time1 = Date.now() - start1;

    // Second request (cache hit)
    const start2 = Date.now();
    const response2 = await axios.get(`${API_URL}/products`);
  ...
  try {
    const response = await axios.get(`${API_URL}/performance/memory-leaks`, {
      headers: { Authorization: `Bearer ${ADMIN_TOKEN}` },
    });
    results.push({
  ...
  try {
    const response = await axios.get(`${API_URL}/performance/slow-queries?threshold=500`, {
      headers: { Authorization: `Bearer ${ADMIN_TOKEN}` },
    });
    results.push({
