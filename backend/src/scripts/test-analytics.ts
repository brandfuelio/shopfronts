...
async function testAnalyticsTracking() {
  ...
  for (const productId of productIds) {
    try {
      await axios.post(
        `${API_URL}/analytics/track/product/${productId}`,
        {
          referrer: faker.internet.url(),
          duration: faker.number.int({ min: 5000, max: 60000 }),
        },
        {
          headers: {
  ...
  for (const query of searchQueries) {
    try {
      await axios.post(
        `${API_URL}/analytics/track/search`,
        {
          query,
          resultsCount: faker.number.int({ min: 0, max: 100 }),
          clickedResults: faker.helpers.arrayElements(productIds, faker.number.int({ min: 0, max: 3 })),
        },
        {
          headers: {
            'X-Session-ID': sessionId,
...
async function testAnalyticsRetrieval() {
  ...
  try {
    const response = await axios.get(`${API_URL}/analytics/users/user_1`, {
      headers: { Authorization: `Bearer ${authToken}` },
    });
...
async function testMonitoring() {
  ...
  try {
    const response = await axios.get(`${API_URL}/monitoring/performance?hours=1`, {
      headers: { Authorization: `Bearer ${authToken}` },
    });
  ...
  try {
    await axios.post(
      `${API_URL}/monitoring/alerts/test`,
      {
        type: 'Test Alert',
