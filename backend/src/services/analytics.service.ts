...
export class AnalyticsService {
   ...
   * Track product view
   */
  static async trackProductView(view: ProductView): Promise<void> {
    try {
      ...
      logger.error('Failed to track product view', { error, view });
    }
  }
   ...
   * Track search query
   */
  static async trackSearch(search: SearchQuery): Promise<void> {
    try {
      await this.trackEvent({
      ...
      logger.error('Failed to track search', { error, search });
    }
  }
  ...
  static async trackConversion(data: {
    ...
    try {
      ...
      logger.error('Failed to track conversion', { error, data });
    }
  }
   ...
   */
  static async getDashboardAnalytics(sellerId?: string): Promise<any> {
    try {
      ...
      const revenueOverTime = await prisma.$queryRaw`
        ...
        ${sellerId ? prisma.$queryRaw`AND id IN (
