...
const seedDatabase = async () => {
  try {
    ...
    await Promise.all(
      reviewData.map(data => prisma.review.create({ data }))
    );
    ...
    logger.info('   Admin: admin@shopfronts.com / password123');
    logger.info('   Seller: seller@shopfronts.com / password123');
    logger.info('   Customer: customer@shopfronts.com / password123');
